...
export function addDefaultHeaders(response: Response) {
  ...
  response.headers.set(
    ...
    `default-src 'none'; script-src 'self' 'unsafe-eval' cdn.ampproject.org ajax.cloudflare.com static.cloudflareinsights.com *.algolia.net *.algolianet.com mc.yandex.ru; style-src 'self' 'unsafe-inline'; img-src 'self' blog-images.clickhouse.tech data: mc.yandex.ru; connect-src 'self' mc.yandex.ru cdn.ampproject.org *.algolia.net *.algolianet.com *.ingest.sentry.io; child-src blob: mc.yandex.ru; frame-src blob: mc.yandex.ru www.youtube.com; font-src 'self' data:; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; prefetch-src 'self'`,
  );
